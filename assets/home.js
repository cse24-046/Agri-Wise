...
grid.innerHTML=HH.seasonalProduce().map(p=>`<article class="produce-card"><div class="produce-icon">${p.name[0]}</div><h4>${p.name}</h4><strong>P${p.price}</strong><span> / ${p.unit}</span><small>Soil: ${p.soil}</small></article>`).join("");
});
